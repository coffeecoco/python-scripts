...
CONFIGURESNMPV3(){
	...
	mv $1 $1.`date +%F-%H%M%S`
	# Replace xxxxxxxxxxxxxxxx with the two different strong passwords
	/usr/bin/net-snmp-config --create-snmpv3-user -ro -A MD5 -a xxxxxxxxxxxxxxxx -X AES -x xxxxxxxxxxxxxxxx WhatsUpMD5AES
	chkconfig snmpd on
...
if [ $? -ne 0 ] ; then
	pidof vmware-guestd
	if [ $? -ne 0 ] ; then
		echo "Please check either VMware Tools or OMSA not installed / running."
		exit 1
	fi
else
